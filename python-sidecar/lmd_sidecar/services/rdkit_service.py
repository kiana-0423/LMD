...
def require_rdkit() -> dict[str, Any]:
    """RDKit has no honest substitute, so its absence is an error rather than a fallback."""
    rdkit = safe_import_rdkit()
    if not rdkit["ok"]:
        raise RuntimeError(
            "RDKit is required but is not available in the packaged sidecar: "
            f"{rdkit['error']}"
        )
    return rdkit
...
def calculate_sketcher_descriptors(smiles: str) -> tuple[dict[str, Any], list[str]]:
    ...
    try:
        ...
        return {
            ...
            "descriptors": descriptors,
            "preview": preview,
            "rdkit_status": "calculated",
            "mordred_status": "calculated",
        }, warnings
    except Exception as exc:
        return {
            "valid": False,
